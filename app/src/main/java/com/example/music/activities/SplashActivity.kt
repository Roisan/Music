        ...
        when(requestCode){
            131->{
                if(grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED
                    ...
                    Toast.makeText(this@SplashActivity,"Please grant all the permissions ",Toast.LENGTH_SHORT)
                    this.finish()
                }
                return
            }
            else->{
                Toast.makeText(this@SplashActivity,"Something is wrong", Toast.LENGTH_SHORT).show()
                this.finish()
                return
            }
        }
